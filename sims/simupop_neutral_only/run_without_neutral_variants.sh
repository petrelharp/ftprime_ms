...
do
    ...
    do
        ...
        then
            ...
            DETAILED_TIME_FILE=simupop_detailed_time_arg_all_neutral.N$N"."size$size".out.gz"
            LOG_FILE=simupop_log_arg_all_neutral.N$N"."size$size".log"
            /usr/bin/time -f "%e %M" -o $TIME_MEM_FILE parallel --timeout $SECONDS_TO_KILL python ../benchmark-simuPOP.py --popsize $N --theta $size --rho $size --nsam 100 --pdel 0.0 --gc $GC --outfile1 $DETAILED_TIME_FILE --logfile $LOG_FILE --seed ::: 42
