...
do
    ...
    do
        ...
        then
            GC=1000
            TIME_MEM_FILE=time_neutral.N$N"."size$size".out"
            DETAILED_TIME_FILE=simupop_detailed_time_neutral.N$N"."size$size".out.gz"
            LOG_FILE=simupop_log_neutral.N$N"."size$size".log"
            /usr/bin/time -f "%e %M" -o $TIME_MEM_FILE parallel --timeout $SECONDS_TO_KILL python benchmark-simuPOP.py --popsize $N --theta $size --rho $size --nsam 100 --pdel 0.01 --gc $GC --outfile1 $DETAILED_TIME_FILE --logfile $LOG_FILE --record-neutral --seed ::: 42
