...
% Thus far what little analytical progress has been made understanding
% how potentially many functional genomic variants might evolve in ecologically realistic environments
% are hard to check due to a lack of realistic simulation methods.
...
% To do this, to \texttt{merge} a record \texttt{(new.left,new.right,parent,new.children)}
% into an existing sorted list of \texttt{records},
% we do the following.
% \begin{enumerate} \setcounter{enumi}{-1}
%     \item Initialize \texttt{here=new.left} and \texttt{k=0}.
%     \item While \texttt{here<right}, let the next entry of \texttt{records} 
%         (ordered by left endpoint) be \texttt{(left,right,parent,children)}.
%         \begin{enumerate}
%             \item If \texttt{right<here}, go to the next record.
%             \item If \texttt{here<left}: 
%                 \begin{enumerate}
%                     \item insert a new record \texttt{(here,min(new.right,left),parent,new.children)} before the current record
%                     \item and set \texttt{here=min(new.right,left)}.
%                 \end{enumerate}
%             \item otherwise, if  TOO MANY STEPS
%         \end{enumerate}
% \end{enumerate}
