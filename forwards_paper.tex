...
We use tree sequences for the substantial algorithmic advantages exploited in~\citep{kelleher2016efficient}.
...
and analyze its run time complexity.


%%%%%%
\subsection*{Simulation benchmarks}

To measure the performance gains from ancestry recording, we ran simulations implemented in C++ (see below) with and without ancestry tracking.  
The simulations without ancestry tracking introduced neutral mutations at a rate 
...
We categorize simulations in terms of the \emph{scaled} parameters
$\theta = 4N\mu$ and $\rho = 4Nr$, where $N$ is the diploid population size,
since these most strongly determine run time.

We ran one simulation for each combination of $N$ from $(1000, 10000, 50000)$, and $\rho$ from $(10^3, 10^4, 10^5)$.
Deleterious mutations were introduced at rate $\rho/100$ per generation, 
drawing scaled selection coefficients ($2Ns$)
from a Gamma distribution with a mean of -5 and a shape parameter of 1.  
This distribution of fitness effects results in
thousands of weakly-deleterious mutations segregating in the population, 
many of which drift to intermediate frequencies.  
The case of many mutations with selection is a non-trivial violation of exchangeability assumptions of the
coalescent \citep{Neuhauser1997-nn}.
Therefore, these selected mutations must be explicitly tracked in our forward simulation,
and the time savings due to ancestry tracking come from not having to record \textit{neutral} mutations.
...
For efficiency reasons, we only ran \simupop{} with ancestry tracking.

\krt{Jaime--any details here to add re: how simuPOP was tweaked??}
...
\begin{figure}
    ...
    \caption{
        ...
        defined by \msprime.
        \plr{Whoops!  Genotypes in the second tree should be `GGC'}
...
% (and all such instances is independent).  % this is understood here
...
%%%%%%%%%%%%%%
\subsection*{Sequential simplification and prior history}
...
as shown in Figure~\ref{fig:num_edges}.
% by comparison to
...
\plr{Replace this with Jerome's implementation.}
