...
generation Wright-Fisher model.
...
so $\mu = r$, where $\mu$ and $r$ are the expected number of mutations and recombinations,
respectively, per gamete and per generation.
...
We ran simulations with different values of $N$
and of the \emph{scaled} parameter $\rho = 4Nr$,
categorized by the value of $\rho$, since this
is what most strongly determines run time
(along with the scaled mutation rate, $\theta = 4N\mu$, which is also equal to $\rho$.)
...
\begin{enumerate}
    ...
    \item
        ...
        using a continuum-sites model for both mutation and recombination.
...
neutral mutations (instead of the ARG), run times increase dramatically with increasing region size ($4Nr = 4Nu$ for
these simulations).  With \fwdpy{}, simulations with $N=5 \times 10^4$ timed out for region sizes larger than $10^3$.
With \simupop{}, we were only able to run simulations with $N=10^3$ and region sizes up to $10^4$.  ARG tracking
...
the ARG, the relative speedup due to ARG tracking is up to $\approx 50$ fold (Figure~\ref{fig:relative_speedup_selection}).  For \fwdpy{}, ARG tracking reduces runtimes more for larger population sizes
(Figure~\ref{fig:relative_speedup_selection}).  We could not make such an observation with \simupop{}, which tended to
run out of memory on our system.  Note the log scale of the x axis in Figure~\ref{fig:runtimes_selection}, which
partially obscures an important observation that the run times appear approximately linear in region size when ARG
tracking.  The reason for this behavior is that the simulations are generating a constant number of new edges on average
each generation, and the number of edges is a function of $\rho$ in expectation.
...
(Figure~\ref{fig:runtimes_selection}).  The relative improvement due to ARG tracking is again substantial in these
simulations (Figure~\ref{sfig:speedup_nosel}).  In fact, we see more of a benefit to ARG tracking here with \fwdpy11{}
...
panels shows run times tracking the ARG instead of the neutral mutations. The dashed line in the upper right panel shows
the results for an implementation using \fwdpy{} where the ARG simplification steps were handled in a separate thread of
execution and fitness calculations were parallelized.}
\end{figure}

\begin{figure}
    \includegraphics[]{sims/speedup}
    \caption{\label{fig:relative_speedup_selection}Relative performance improvement due to ARG tracking. This figure
    shows the ratio of the run time tracking neutral mutations over the run time tracking the ARG. Data points are taken
...
adds a new edge $(\ell, r, p, c)$ to the edge table $\Et$.
...
\begin{figure}
    ...
    \caption{\label{sfig:speedup_nosel}Relative performance improvement due to ARG tracking for simulations without
    selection.  Data points are taken from Figure~\ref{sfig:rawspeed_nosel} and show the ratio of run times tracking
neutral variation over the run times tracking the ARG.}
\end{figure}
