...
Therefore, the total area of a tree sequence is equal to the sequence length
multiplied by the mean area of a marginal tree.
Since we analyze tree sequences arising from a Wright--Fisher model,
statistical properties of a marginal tree are well-described by coalescent theory.
...
which requires at most $N-2$ edges.
...
Suppose that $T_x$ is the marginal tree describing how each of the present-day population
descends from their most recent common ancestor
at a particular genomic position $x$ on the chromosome,
and write $|T_x|$ for the total length of the tree,
i.e., the number of ancestors 
from which some but not all of the present-day population has inherited at position $x$
(including the population itself).
For the tree at nearby position $x + dx$ to be different,
there must have been a crossing-over between $x$ and $x + dx$
in one of the $|T_x|$ meioses that gave birth to those individuals.
...
the expected distance until the next crossing-over is $1/|T_x|$.
...
If $T$ is large enough that all samples coalesce within the simulation with high probability
-- $T = 20N$, say --
...
is expected to have around $r(T)$ roots, where $r(T) \approx 2N/(T+2)$.
The total tree length over this time is
$2N \sum_{k=N}^{r(T)+1} 1/(k-1) \approx 2 N \log(N/r(T))$.
...
there would also be around $\mu N T$ mutations, and the same number of sites.
