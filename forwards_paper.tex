...
general problem that cover in the next section.
...
processing each edge in the input tree sequence in chronological order.
The main state of the algorithm at each point in time is a set of ancestral segments:
each segment $(\ell, r, u)$ represents a tract of ancestral material in which the node $u$ is
associated with the genomic interval $[\ell, r)$. 
...
The node $u$ in each ancestral sequence
...
\plr{I am trying it out to see what it looks like.}
...
\algstep{S2.}{Process parent edges.}{Loop over edges associated with this parent, removing ancestry from each child.
}
...
\begin{taocpalg}{R}{Remove ancestry}
    {Remove ancestry on a segment from an ancestor.}
