...
days of CPU time simply to read the information info memory. \jk{Could
add concrete timing estimates based on subsets here, but maybe overdoing it?}

The tree sequence encoding is very compact but it is also very efficient to process.
...
Set $E\leftarrow \emptyset$ and $n \leftarrow N$.
}

\algstep{W2.}{Generation loop.}{ If $t = 0$ terminate.
Set $t \leftarrow t - 1$ and $j \leftarrow 0$.
}

\algstep{W3.}{Individual loop.}{ If $j = N$ set $P \leftarrow P'$ and go to \algref{W2}.
}

\algstep{W4.}{Mortality.}{ If $\randomuniform([0, 1)) > \delta$, set $P'_j \leftarrow P_j$
    and go to \algref{W7}.
}
...
\algstep{W8.}{Loop tail}{Set $j \leftarrow j + 1$ and go to \algref{W4}.
}
...
\begin{enumerate}
\item Explain how to add mutational information. We can also do this afterwards on the
output trees if they are nuetral.
\item Explain that we can input these trees into an mpsrime tree sequence and they will
work fine. Perhaps a diagram to show the kind of trees we get? (Multiroots, lots of unary nodes.
Can use the msprime SVG API to draw these?) We could refer ahead to figure 2
instead though.
\item However, there is huge amount of redundancy in these. The amount of information stored
