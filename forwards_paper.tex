...
\newcommand{\priorityq}[1]{\mbox{PriorityQueue}({#1})}
\newcommand{\listnew}[1]{\mbox{List}(#1)}
\newcommand{\pqpush}[2]{#1.\mbox{push}(#2)}
\newcommand{\pqpop}[1]{#1.\mbox{pop}()}
\newcommand{\listappend}[2]{#1.\mbox{append}(#2)}
\newcommand{\ancsegment}[1]{\mbox{Segment}(#1)}
\newcommand{\taddrow}[2]{#1.\mbox{addrow}(#2)}
...
Call $\taddedge{\Et, 0, x, P_a, u}$ and $\taddedge{\Et, x, L, P_b, u}$.
}
...
\algstep{S2.}{Insert edge/ancestry intersections.}{
    ...
    $y \leftarrow \ancsegment{\ell, r, \attrid{x}}$ and call
    $\pqpush{Q}{y}$.
}
...
\algstep{S4.}{No overlapping segments.}{
    ...
    \ancsegment{\attrleft{x}, \attrright{Q[0]}, \attrid{x}}$,
...
\algstep{S6.}{Record edges.}{
    ...
    call $\taddrow{\Et_O}{l, r, v, \attrid{x}}$ and, if $\attrright{x} > r$,
    set $\attrleft{x} \leftarrow r$ and call $\pqpush{Q}{x}$.
}

\algstep{S7.}{Left coordinate loop.}{
    Call $\listappend{\Al}{\alpha}$. If $|Q| > 0$ return to \algref{S3}.
}
