...
to find overlaps (and hence coalescences), and to update the $\Al$.
...
\algstep{S1.}{Select parent edges.}{
    ...
    Set $v \leftarrow -1$.  If $|E| = 0$, go to \algref{S8}.
}
...
\algstep{S5.}{Overlap; new output node.}{
...
The algorithm begins in \algref{S0} by allocating an empty list to store ancestral
...
To step back up through the pedigree, steps \algref{S1}--\algref{S8}
...
Then, in \algref{S2} we find all intersections between these input edges and the
...
Step \algref{S3} builds a list $X$ of all segments covering the current left-most coordinate
...
and the ancestry segment $\alpha$ is not changed (this is assigned to $\Al_u$ in \algref{S7}).
...
After doing this, we proceed to \algref{S7}, update the ancestry mapping for
