...
\author{Jerome Kelleher\footnote{Big Data Institute, University of Oxford},
        Kevin R. Thornton\footnote{Ecology and Evolutionary Biology, University of California at Irvine},
        Jaime Ashander\footnote{Ecology and Evolutionary Biology, University of California at Los Angeles}, and
...
\begin{abstract}
    ...
    Recording the results of each breeding event
    results in a linear growth in the amount of information that must be stored.
    However, there is a great deal of redundancy in this information,
    since the majority of individuals simulated over the course of the simulation
    will leave no descendents in the eventual sample.
    ...
    (2) A population of $N$ individuals with $M$ polymorphic sites
        ...
        (and its history) rather than only a subset.
    (3) A simulation can easily be initialized with a more efficient coalescent simulation 
        of deep history.
    The software for recording and processing tree sequences is named \tskit.
\end{abstract}
...
only requiring a little one-way output at the birth of each new individual.
...
\begin{itemize}
    ...
    \item and the derived state of these new mutations in the Mutation Table
        (as well as the identiy of this genome the mutations appeared in).
\end{itemize}
...
while here they are predetermined by history.
...
\begin{figure}
    ...
    \end{center}
    \caption{
        A depiction of each state of the simplification algorithm
...
could in principle generate tables of nodes and edges to be simplified in \tskit{}.  The resulting succinct tree
sequence object would be in the same format as those generated by \tskit{}'s built-in simulate function, and therefore
...
that manages communication between the forward simulator and \tskit{}.
...
As for the \fwdpp{} simulations above, we set scaled mutation rate $\theta=4N\mu=\rho$
(where $\mu$ is the per-site mutation rate multiplied by the number of sites),
and used a simplify interval of 1000 generations and simulated for $T=10N$ generations in each
simulation.
...
We coupled \simupop{} to \tskit{} using a Python class \texttt{RecombCollector} that 1)
...
\texttt{tskit.sort\_tables()} and \texttt{tskit.simplify\_tables}).
The tables are initialized using the \texttt{TreeSequence} returned from
\texttt{tskit.simulate($2N$, Ne=$N$, recombination\_rate=$r/2$, length=$n$)}.
...
% with conda, but \tskit{} must be updated to the version listed below via pip
...
but the version implemented in \tskit{} makes different sortedness assumptions
