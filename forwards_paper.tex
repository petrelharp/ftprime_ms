...
\begin{abstract}
    ...
    and show how to quickly simplify a tree sequence by removing history irrelevant to a given set of genomes.
\end{abstract}
...
for recording tree sequences succinctly in several \emph{tables};
(b) an algorithm to record these tables on the fly during a forwards-time simulation;
...
and a numerically larger speedup to pedigree tracking (see Appendix~\ref{ss:timing_nosel}).
...
This proof-of-concept implementation, the simulation engine (we use \simupop{}, \citet{simupop})
...
adds a new edge $(\ell\mbox{eft}, r\mathrm{ight}, p\mathrm{arent}, c\mathrm{hild})$ to the edge table $\Et$.
...
by simply recording the node in which they first occur, the derived state,
...
\paragraph{Relationship to the ARG}
...
which would allow highly efficient computation of properties of the posterior distribution.
...
Here we describe the simplification algorithm. The algorithm uses a few simple structures
...
$\attrleft{x}$ and $\attrright{x}$ represents the genomic interval in the usual way,
and $\attrnode{x}$ represents the output node ID that is assigned to this interval. The per-interval
segments are assigned to input IDs via the $\Al$ mapping, which is the main state required
by the algorithm. Each element $\Al[u]$
is a list of ancestral segments, describing the output node ID mapped to input
node ID $u$ over a set of intervals. For each input node, we need to find the process
the resulting set of ancestral segments to find overlaps (and hence coalescences). We
do this using a $\priorityq{}$ type. This priority queue maintains a list of ancestral
segments in sorted order, such that the segements with the smallest left coordinate
are at the start of the list. Given a priority queue $Q$, the operation
$\pqpush{Q}{x}$ inserts the segment $x$ into the queue, and $\pqpop{Q}{x}$
returns (and removes) the segment in $Q$ with the smallest left coordinate.
To obtain the segment currently in the queue with the smallest left value we
write $Q[0]$.

A step-by-step explanation of the algorithm follows the algorithm listing.
...
\algstep{S1.}{Select parent edges.}{
    Set $E \leftarrow \{e: \attrparent{e} = u \mbox{ and } e \in \Et_I\}$.
    Set $v \leftarrow -1$.
}

\algstep{S2.}{Insert edge/ancestry intersections.}{
    For each $e\in E$ and each $x \in \Al[\attrchild{e}]$,
    if $\attrright{x} > \attrleft{e}$ and $\attrright{e} > \attrleft{x}$,
    set $\ell \leftarrow \max(\attrleft{x}, \attrright{e})$,
    $r \leftarrow \min(\attrright{x}, \attrright{e})$,
    $y \leftarrow \ancsegment{\ell, r, \attrnode{x}}$ and call
    $\pqpush{Q}{y}$.
}

\algstep{S3.}{Find segments with minimum left coordinate.}{
...
\algstep{S4.}{No overlapping segments.}{
...
\algstep{S5.}{New output node.}{
    If $v = -1$, set $v \leftarrow \taddrow{\Nt_0}{\Nt_I[u]}$.
}

\algstep{S6.}{Record edges.}{
...
\algstep{S7.}{Left coordinate loop.}{
    Call $\listappend{\Al[u]}{\alpha}$. If $|Q| > 0$ return to \algref{S3}.
}

\algstep{S8.}{Parent loop.}{
    Set $u \leftarrow u + 1$. If $u < |\Nt_I|$, go to \algref{S1}.
}
...
The algorithm steps back up through the pedigree and steps \algref{S1}--\algref{S8}
consider each node $u$ in turn (note we are assuming that nodes are ordered by time-since-birth).
In \algref{S1} we first find all edges in the input where the parent is equal to
our current input ID $u$, and then set the corresponding output ID $v$ to $-1$.
Then, in \algref{S2} we find all intersections between these edges and the
mapped ancestry segments for their child nodes. For each of these intersections we
...
segments, we then need to process these segments. Steps \algref{S3}--\algref{S7}
loop over the segments in the queue, considering each in turn until the queue is empty.
Step \algref{S3} builds a list $X$ of all the segments that have the
minimum left coordinate (along with some bookkeeping to keep track of where our next
...
we proceed to \algref{S4}.

As no overlaps occur, there is no change in the mapping between input and output IDs
and so the ancestry segment $\alpha$ is not changed (this is assigned to $\Al_u$ in \algref{S7}).
However, there is a complication caused by other segments which may intersect with $x$,
which have left coordinate greater then $\attrleft{x}$. If $x$ intersects with this value,
then there are other overlaps we must consider. Therefore, we change the left coordinate
of $x$ to this coordinate and reinsert it into $Q$ where it can be processed again
later. After doing this, we proceed to \algref{S7}, update the ancestry mapping for
the input node $u$, and loop back to \algref{S3} if $Q$ is not empty.

On the other hand, if there is more than one segment in $X$, we know that an
overlap among the ancestral segments has occured and we must update the output
nodes and edges accordingly. In step \algref{S5} we first allocate a new output
node $v$ for $u$, if it has not been done already (we may have many overlapping
segments along the genome). Step \algref{S6} then continues by creating a new
